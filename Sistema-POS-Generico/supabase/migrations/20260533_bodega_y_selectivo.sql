...
BEGIN
    ...
    IF v_tipo = 'SELECTIVO' OR v_tipo = 'AJUSTE_DIRECTO' THEN
        RETURN QUERY
        WITH conteo AS (
            SELECT ic.maestro_producto_id, COALESCE(SUM(ic.cantidad_escaneada * ic.factor_conversion), 0) as total_fisico
            FROM public.inventory_counts ic
            JOIN public.maestro_productos mp ON ic.maestro_producto_id = mp.id
            WHERE ic.session_id = p_session_id AND mp.empresa_id = v_empresa_id
            GROUP BY ic.maestro_producto_id
        ),
        sistema AS (
            SELECT p.maestro_producto_id, COALESCE(SUM(p.stock_actual), 0) as total_sistema
            FROM public.productos p
            WHERE p.empresa_id = v_empresa_id AND p.stock_actual > 0
              AND (v_bodega_id IS NULL OR p.bodega_id = v_bodega_id)
            GROUP BY p.maestro_producto_id
        )
        SELECT 
            c.maestro_producto_id, mp.nombre,
            COALESCE(s.total_sistema, 0), c.total_fisico,
            (c.total_fisico - COALESCE(s.total_sistema, 0)),
            ((c.total_fisico - COALESCE(s.total_sistema, 0)) * COALESCE(mp.precio_venta, 0))
        FROM conteo c
    ...
    ELSE
        ...
        SELECT 
            COALESCE(c.maestro_producto_id, s.maestro_producto_id), mp.nombre,
            COALESCE(s.total_sistema, 0), COALESCE(c.total_fisico, 0),
