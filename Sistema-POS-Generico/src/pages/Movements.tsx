...
export function Movements() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      await supabase.from('users').select('id, email').order('email');
    };
    fetchUsers();
  }, []);
  ...
  useEffect(() => {
    const fetchMovements = async () => {
      ...
      try {
        ...
        toast.error(`Error al cargar movimientos: ${error instanceof Error ? error.message : error}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleExport = () => {
    ...
    const dataToExport = movements.map(mov => ({
      'Fecha': mov.creado_en ? format(new Date(mov.creado_en), 'dd/MM/yyyy HH:mm:ss') : 'S/F',
      'Producto': mov.producto_nombre,
      'Origen/Destino': mov.origen_destino || '-',
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Origen/Destino</th>
                ...
                movements.map((mov, index) => (
                  <tr key={index}>
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-700">
                      {mov.creado_en ? format(new Date(mov.creado_en), 'dd/MM/yy HH:mm', { locale: es }) : 'S/F'}
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{mov.producto_nombre}</td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-700">{mov.origen_destino || '-'}</td>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-700">{mov.usuario_nombre || 'Sistema'}</td>
